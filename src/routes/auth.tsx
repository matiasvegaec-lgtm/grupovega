...
import { Loader2, Heart, Package, Mail, Sparkles, Check } from "lucide-react";
...
function AuthPage() {
  ...
  const inputCls =
    "w-full px-4 py-3 rounded-xl bg-white border-2 border-border focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/10 transition text-navy-deep placeholder:text-muted-foreground";
      ...
      <section className="min-h-screen pt-24 pb-16 bg-gradient-to-b from-foam via-background to-foam">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-white rounded-3xl p-7 sm:p-9 shadow-elegant border border-border">
            ...
            <p className="text-sm text-ocean mb-6">
              {isLogin ? "Inicia sesión o crea una cuenta" : "Regístrate para comprar y guardar favoritos"}
            </p>
            ...
            <button
              ...
              disabled={submitting}
              className="w-full inline-flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-white border-2 border-border text-navy-deep font-semibold hover:bg-foam transition disabled:opacity-60"
            >
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer pt-1 select-none">
                  <button
                    ...
                    className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition ${
                      acceptPromos ? "bg-ocean border-ocean" : "bg-white border-border"
                    }`}
                    aria-label="Acepto recibir promociones"
                  >
                    {acceptPromos && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                  </button>
              ...
              <p className="text-xs text-center text-muted-foreground pt-2">
                ...
                <Link to="/contacto" className="underline hover:text-ocean">Términos del servicio</Link>
              </p>
            </form>
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            <Link
              to="/favoritos"
              className="bg-white rounded-2xl p-4 border border-border hover:border-ocean hover:shadow-card transition text-center group"
            >
              <Heart className="w-5 h-5 mx-auto mb-1.5 text-orange-500 group-hover:scale-110 transition" />
              <div className="text-xs font-semibold text-navy-deep">Favoritos</div>
            </Link>
            <Link
              to="/mis-pedidos"
              className="bg-white rounded-2xl p-4 border border-border hover:border-ocean hover:shadow-card transition text-center group"
            >
              <Package className="w-5 h-5 mx-auto mb-1.5 text-ocean group-hover:scale-110 transition" />
              <div className="text-xs font-semibold text-navy-deep">Mis pedidos</div>
            </Link>
            <div className="bg-white rounded-2xl p-4 border border-border text-center">
              <Sparkles className="w-5 h-5 mx-auto mb-1.5 text-turquoise" />
              <div className="text-xs font-semibold text-navy-deep">Promos</div>
            </div>
          </div>
