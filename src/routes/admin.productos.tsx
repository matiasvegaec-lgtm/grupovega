...
export const Route = createFileRoute("/admin/productos")({
  head: () => ({
    meta: [
      { title: "Admin · Productos — Grupo Vega" },
...
const getPresentationParts = (presentation: string | null | undefined) => {
  ...
  return {
    ...
    unit:
      /(^|\s)g$/i.test(value) && !/kg/i.test(value)
        ? ("g" as PresentationUnit)
        : ("kg" as PresentationUnit),
  };
};

const formatPresentation = (amount: string, unit: PresentationUnit) =>
  amount === "" ? "" : `${amount} ${unit}`;
...
function AdminProductos() {
  ...
  const openEdit = (p: Product) => {
    ...
    setForm({
      ...
      protein_content: p.protein_content ?? "",
      price_card_3m: p.price_card_3m ?? null,
    });
    setShowForm(true);
  };
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        protein_content: form.protein_content || null,
        price_card_3m:
          form.price_card_3m === null ||
          form.price_card_3m === undefined ||
          (form.price_card_3m as any) === ""
            ? null
            : Number(form.price_card_3m),
      };
      if (editing) {
  ...
  const inputCls =
    "w-full px-3 py-2 rounded-lg bg-background border border-border focus:border-ocean focus:outline-none text-sm";
  ...
  return (
    <div className="p-4 md:p-8 pb-24 md:pb-8">
      <div className="flex justify-between items-start md:items-center mb-6 gap-3">
        ...
        <button
          onClick={openNew}
          className="hidden md:inline-flex items-center gap-2 px-4 py-2.5 rounded-full gradient-wave text-white font-semibold shadow-glow"
        >
      ...
      <button
        onClick={openNew}
        className="md:hidden fixed bottom-6 right-6 z-20 w-14 h-14 rounded-full gradient-wave text-white shadow-glow flex items-center justify-center"
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-card rounded-2xl">
          ...
          <button
            onClick={openNew}
            className="px-6 py-2.5 rounded-full gradient-wave text-white font-semibold"
          >
            Crear el primero
          </button>
        </div>
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {filteredList.length === 0 ? (
              <div className="text-center py-10 bg-card rounded-2xl text-sm text-muted-foreground">
                Sin resultados para "{searchName}"
              </div>
            ) : (
              filteredList.map((p) => (
                <div key={p.id} className="bg-card rounded-2xl shadow-card p-3 flex gap-3">
                  {p.image_url ? (
                    <ProductImage
                      src={p.image_url}
                      alt={p.name}
                      className="w-20 h-20 rounded-xl object-contain shrink-0 bg-transparent"
                    />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-2">
                      ...
                      <button
                        ...
                        className={`shrink-0 p-1 -mt-1 rounded ${p.featured ? "text-yellow-500" : "text-gray-300"}`}
                      >
                    ...
                    <div className="flex items-center gap-2 mt-1.5 text-xs">
                      ...
                      <span
                        className={`ml-auto px-2 py-0.5 rounded-full font-semibold ${p.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                      >
                    ...
                    <div className="flex gap-1.5 mt-2">
                      <button
                        onClick={() => openEdit(p)}
                        className="flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg bg-foam text-ocean text-xs font-semibold"
                      >
                        <Pencil className="w-3.5 h-3.5" /> Editar
                      </button>
                      <button
                        onClick={() => toggleActive(p)}
                        className="px-2 py-1.5 rounded-lg bg-foam text-navy-deep"
                      >
                        {p.active ? (
                      ...
                      <button
                        ...
                        className="px-2 py-1.5 rounded-lg bg-destructive/10 text-destructive"
                      >
                    ...
                    <tr key={p.id} className="border-t border-border">
                      <td className="p-3">
                        {p.image_url ? (
                          ...
                          <div className="w-12 h-12 rounded-lg bg-foam" />
                        )}
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition ${p.featured ? "text-yellow-500 hover:bg-yellow-50" : "text-gray-300 hover:bg-foam"}`}
                        >
                          <Star className={`w-4 h-4 ${p.featured ? "fill-current" : ""}`} />
                        </button>
                      </td>
                      <td className="p-3 text-center">
                        <button
                          onClick={() => toggleActive(p)}
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${p.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                        >
                      ...
                      <td className="p-3 text-right">
                        <button
                          onClick={() => openEdit(p)}
                          className="p-2 rounded-lg hover:bg-foam text-ocean"
                        >
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDelete(p)}
                          className="p-2 rounded-lg hover:bg-foam text-destructive"
                        >
      ...
      {showForm && (
        ...
        >
          <form
            ...
            className="bg-card rounded-t-2xl md:rounded-2xl p-4 md:p-6 max-w-2xl w-full max-h-[92vh] md:max-h-[90vh] overflow-auto shadow-elegant"
          >
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  value={form.category}
                  onChange={(e) =>
                    setForm({ ...form, category: e.target.value, subcategory_id: null })
                  }
                  className={inputCls}
                >
                  {categories.length === 0 && (
              ...
              <div>
                ...
                {(() => {
                  ...
                  const subs = parent
                    ? subcategories.filter((s) => s.category_id === parent.id)
                    : [];
                  return (
                    <select
                      ...
                      className={
                        inputCls + (subs.length === 0 ? " opacity-50 cursor-not-allowed" : "")
                      }
                    >
                      <option value="">
                        {subs.length === 0 ? "Sin subcategorías" : "— Ninguna —"}
                      </option>
                      {subs.map((s) => (
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      setForm({
                        ...form,
                        presentation: formatPresentation(e.target.value, presentationParts.unit),
                      });
                    }}
                    placeholder="25"
                    ...
                    onChange={(e) => {
                      setForm({
                        ...
                        presentation: formatPresentation(
                          presentationParts.amount,
                          e.target.value as PresentationUnit,
                        ),
                      });
                    }}
              ...
              <div>
                <label className="text-xs font-semibold text-navy-deep">
              ...
              <div className="sm:col-span-2">
                <label className="text-xs font-semibold text-navy-deep">
                  Precio con tarjeta a 3 meses (USD)
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-muted-foreground pointer-events-none">
                    $
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        price_card_3m: e.target.value === "" ? null : parseFloat(e.target.value),
                      })
                    }
                ...
                <p className="text-[11px] text-muted-foreground mt-1">
                  Si lo dejas vacío no se mostrará el precio diferido.
                </p>
              </div>
              <div className="sm:col-span-2">
                ...
                <div className="flex gap-3 items-start">
                  {form.image_url && (
                    <ProductImage
                      src={form.image_url}
                      alt=""
                      className="w-20 h-20 rounded-lg object-contain bg-transparent"
                    />
                  )}
                  <div className="flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1.5">
                      La imagen se procesa automáticamente al subirla y luego puedes reajustarla si
                      necesitas moverla o acercarla.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-3 justify-end mt-6">
              ...
              <button
                disabled={saving}
                type="submit"
                className="px-6 py-2 rounded-lg gradient-wave text-white text-sm font-semibold inline-flex items-center gap-2 disabled:opacity-60"
              >
