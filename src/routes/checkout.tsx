...
function CheckoutPage() {
  ...
  const validateStep = (idx: number): boolean => {
    ...
    if (idx === 0) {
      ...
      else if (!/^[\d+\s-]{7,15}$/.test(form.customer_phone.trim()))
  ...
  const buildOrderSummaryText = (orderNumber: string) => {
    ...
    const tipoDoc =
      method === "quote"
        ? "📄 *Cotización*"
        : method === "card"
          ? "💳 *Factura / Link de pago*"
          : method === "cash"
            ? "🏬 *Factura / Pago en efectivo*"
            : "🧾 *Factura / Pedido*";
    const accion =
      method === "transfer"
        ? "Estado: esperando comprobante de pago"
        : method === "card"
          ? "Solicitud: enviar link seguro de pago"
          : method === "cash"
            ? "Solicitud: reservar pedido para pago en sucursal"
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const notes =
        `Método: ${method}. Recibe: ${form.receiver_name}. RUC: ${form.customer_ruc}. ${form.shipping_notes || ""}`.trim();
      ...
      if (method === "transfer") {
        ...
        toast.success(
          "Pedido registrado. Te enviamos los datos por WhatsApp para confirmar el pago con tarjeta.",
        );
    ...
    return (
      <Layout>
        ...
        <section className="py-20 text-center">
          <Loader2 className="w-8 h-8 mx-auto animate-spin text-ocean" />
        </section>
      </Layout>
    );
  }
  ...
  const inputCls =
    "w-full px-4 py-2.5 rounded-lg bg-background border border-border focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/20 transition";
  ...
  return (
    <Layout>
      <PageHero
        eyebrow="Pago seguro"
        title="Checkout"
        description="Completa tus datos y elige el método de pago."
      />
          ...
          <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
                    ...
                    return (
                      <div key={s.key} className="flex items-center flex-1 last:flex-none">
                        <div className="flex flex-col items-center gap-1.5 min-w-0">
                          <div
                            className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition shrink-0 ${
                              done
                                ? "bg-ocean border-ocean text-white"
                                : active
                                  ? "border-ocean text-ocean bg-foam"
                          ...
                          <span
                            className={`text-[11px] sm:text-xs font-semibold truncate ${active || done ? "text-navy-deep" : "text-muted-foreground"}`}
                          >
                            Paso {i + 1}: {s.label}
                          </span>
                        </div>
                        {i < STEPS.length - 1 && (
                          <div
                            className={`h-0.5 flex-1 mx-2 mb-5 transition ${i < step ? "bg-ocean" : "bg-border"}`}
                          />
                        )}
                      </div>
                    );
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <input
                        ...
                        className={`${inputCls} ${errors.customer_name ? errCls : ""}`}
                      />
                    ...
                    <div>
                      <input
                        ...
                        className={`${inputCls} ${errors.receiver_name ? errCls : ""}`}
                      />
                    ...
                    <div>
                      <input
                        ...
                        onChange={update("customer_ruc")}
                        className={`${inputCls} ${errors.customer_ruc ? errCls : ""}`}
                      />
                    ...
                    <div>
                      <input
                        ...
                        className={`${inputCls} ${errors.customer_email ? errCls : ""}`}
                      />
                    ...
                    <div>
                      <input
                        ...
                        className={`${inputCls} ${errors.customer_phone ? errCls : ""}`}
                      />
              ...
              {step === 1 && (
                <div className="bg-card rounded-2xl p-6 shadow-card">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <input
                        placeholder="Lugar o nombre de la camaronera *"
                        value={form.farm_name}
                        onChange={update("farm_name")}
                        className={`${inputCls} ${errors.farm_name ? errCls : ""}`}
                      />
                      {fieldError("farm_name")}
                    </div>
                    <div className="sm:col-span-2">
                      <input
                        placeholder="Referencia (cómo llegar) *"
                        value={form.reference}
                        onChange={update("reference")}
                        className={`${inputCls} ${errors.reference ? errCls : ""}`}
                      />
                ...
                <div className="bg-card rounded-2xl p-6 shadow-card">
                  ...
                  {method === "transfer" && (
                    <div className="mt-5 p-4 rounded-xl bg-foam border border-border space-y-2 text-sm">
                      <p className="font-semibold text-navy-deep">
                        Datos para transferencia (ejemplo):
                      </p>
                      <ul className="space-y-1 text-muted-foreground">
                        <li>
                          <strong>Banco:</strong> {BANCO_DEMO.banco}
                        </li>
                        <li>
                          <strong>Tipo:</strong> {BANCO_DEMO.tipo}
                        </li>
                        <li>
                          <strong>N° de cuenta:</strong> {BANCO_DEMO.numero}
                        </li>
                        <li>
                          <strong>Titular:</strong> {BANCO_DEMO.titular}
                        </li>
                        <li>
                          <strong>RUC:</strong> {BANCO_DEMO.ruc}
                        </li>
                        <li>
                          <strong>Correo:</strong> {BANCO_DEMO.correo}
                        </li>
                      </ul>
                      <p className="text-xs text-ocean flex items-center gap-2 pt-2">
                        <Send className="w-3.5 h-3.5" /> Al confirmar, abriremos WhatsApp con tu
                        resumen para que adjuntes el comprobante (obligatorio).
                      </p>
                    </div>
                  )}
                  {method === "cash" && (
                    <div className="mt-5 p-4 rounded-xl bg-foam border border-border space-y-2 text-sm">
                      ...
                      <p className="text-muted-foreground flex items-start gap-2">
                        <Building2 className="w-4 h-4 mt-0.5 text-ocean" /> {SUCURSAL.direccion}
                      </p>
                      <p className="text-muted-foreground flex items-start gap-2">
                        <Clock className="w-4 h-4 mt-0.5 text-ocean" /> {SUCURSAL.horario}
                      </p>
                      <p className="text-xs text-ocean pt-2">
                        Tu pedido quedará reservado y se despacha al momento del pago.
                      </p>
                    </div>
                  )}
                  {method === "card" && (
                    <div className="mt-5 p-4 rounded-xl bg-foam border border-border space-y-2 text-sm">
                      ...
                      <p className="text-muted-foreground">
                        Al confirmar, enviaremos los datos completos de tu factura por WhatsApp a la
                        empresa. Te responderemos con el link seguro de pago para que completes la
                        transacción con tu tarjeta de crédito o débito.
                      </p>
                      <p className="text-xs text-ocean flex items-center gap-2 pt-2">
                        <Send className="w-3.5 h-3.5" /> Procesamiento seguro mediante pasarela de
                        pagos.
                      </p>
                    </div>
                  )}
                  {method === "quote" && (
                    <div className="mt-5 p-4 rounded-xl bg-foam border border-border space-y-2 text-sm">
                      ...
                      <p className="text-muted-foreground">
                        Al confirmar, te generamos una cotización formal con todos los productos
                        seleccionados y la enviamos a la empresa por WhatsApp. Un asesor te
                        contactará para finalizar los detalles, sin compromiso de compra.
                      </p>
                      <p className="text-xs text-ocean flex items-center gap-2 pt-2">
            ...
            <div className="bg-card rounded-2xl p-6 shadow-card h-fit lg:sticky lg:top-24">
              ...
              <label className="flex items-start gap-2 mt-4 cursor-pointer text-xs text-muted-foreground select-none">
                ...
                />
                <span>
                  Guardar mis datos en este dispositivo para futuras compras (más rápido la próxima
                  vez).
                </span>
              </label>
              <button
...
function MethodOption({
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selected ? "border-ocean" : "border-border"}`}
      >
